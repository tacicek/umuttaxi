// Astro server endpoint that receives the contact + booking forms and sends an
// email via Resend. Replaces the old Netlify Forms + submission-created.js
// serverless function so the site can run anywhere (Coolify, VPS, Docker…).
//
// The forms POST url-encoded data to this route; we normalise it into the same
// `data` shape the old Netlify function used, then reuse the identical email
// templates.
...
function buildEmail(data) {
  ...
  let subject, emailHtml, emailText;
...
export async function POST({ request }) {
  // Native (no-JS) form posts want an HTML page back; fetch()-based posts want
  // JSON. Decide the success response shape from the Accept header.
  const wantsHtml = (request.headers.get('accept') || '').includes('text/html');
  const ok = () =>
    wantsHtml
      ? new Response(null, { status: 303, headers: { Location: '/danke' } })
      : new Response(
          JSON.stringify({ message: 'Form submission processed and email sent successfully' }),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );

  try {
    // Accept both url-encoded form posts and JSON.
    ...
    const { subject, emailHtml, emailText } = buildEmail(data);
    ...
    return new Response(
      JSON.stringify({ message: 'Error processing form submission', error: detail }),
